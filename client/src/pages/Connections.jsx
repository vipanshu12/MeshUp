...
import {dummyConnectionsData as Connection,
    ...
    <div className='min-h-screen bg-slate-50'>
      ...
      <div className='mb-8  flex flex-wrap  gap-6'>
        {dataArray.map((item , index )=>(
          <div key={index} className='flex flex-col items-center justify-center bg-whitegap-1 border h-20 w-40 border-gray-200 bg-white
          shadow rounded-md'>
      ...
      border border-gray-200 rounded-md p-1 shadow-sm'>
        {
          dataArray.map((tab) =>(
            <button onClick={()=> setCurrentTab(tab.label)} key={tab.label} className={` cursor-pointer flex items-center px-3 py-1 text-sm rounded-md p-1 transition-colors ${currentTab === tab.label ? 'bg-white font-medium text-black' : 'text-gray-500 hover:text-black'} `}>
                ...
                <div className='flex max-sm:flex-col gap-2 mt-4 '>
...

                  {
                    currentTab === 'Following' && (
                      <button className='w-full p-2 text-sm 
                      rounded bg-slate-100  hover:bg-slate-200 text-black active:scale-95 
