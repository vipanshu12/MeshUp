  ...
  return (
    ...
    w-72 shadow border border-gray-200 rounded-md '>
        ...
        <div>
            ...
            shadow-md mx-auto' />
            <p className='mt-4 font font-semibold'>{user.full_name}</p>
            {user.username && <p className='text-gray-500 font-light'>@{user.username}</p>}
            {user.bio && <p className='text-gray-60 mt-2 text-center text-sm'>{user.bio}</p>}
        ...
        gap-2 mt-4 text-xs text-gray-600'>
           ...
           text-slate-500 group rounded-md cursor-pointer active:scale-95 transition'>
            {
              ...
              transition'/> :<Plus className='w-5 h-5 group-hover:scale-105 transition'/>
            }
          </button>
        </div>
